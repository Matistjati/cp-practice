...
Node* roots[int(1e5) + 10];
Node* roots2[int(1e5) + 10];
...
Node* update(Node* node, int l, int r, int i, int v)
{
	...
	int mid = (l + r) / 2;
	if (i <= mid) return new Node(update(node->l, l, mid, i, v), node->r);
	else return new Node(node->l, update(node->r, mid + 1, r, i, v));
}
...
int32_t main()
{
	...
	rep(i, n)
	{
		roots[cnt] = update(roots[cnt - 1], 0, n - 1, nums[i].second, nums[i].first);
		roots2[cnt] = update(roots2[cnt - 1], 0, n - 1, nums[i].second, 1);
		cnt++;
	}
	...
	while (q--)
	{
		...
		{
			int mid = (hi + lo) / 2;
			if (query(roots[mid], 0, n - 1, l, r) > v)
			{
				hi = mid;
			}
			else
		...
		cout << query(roots2[lo], 0, n - 1, l, r) << "\n";
	}
