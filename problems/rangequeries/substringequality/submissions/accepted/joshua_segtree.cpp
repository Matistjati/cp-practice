...
struct Tree
{
	...
	Tree(int n, int base) : tree(n * 4), base(base)
	{
		int m = 1;
		rep(i, n)
		{
			powers.push_back(m);
			inverse.push_back(binpow(m, mod - 2, mod));
			m = (m * base) % mod;
		}
	}

	void update(int x, int l, int r, int i, int c)
	{
		if (r<i || l>i) return;
		if (l==r)
		{
			tree[x] = (c*powers[i])%mod;
		}
		else
		{
			int mid = (l + r) / 2;
			update(x * 2, l, mid, i, c);
			update(x * 2+1, mid+1, r, i, c);
	...
	int query(int x, int l, int r, int ql, int qr)
	{
		...
		if (l >= ql && r <= qr) return (tree[x] * inverse[ql])%mod;
		int mid = (l + r) / 2;
...
int32_t main()
{
	...
	int bases[4] = {157,163,2143,6967};
	vector<Tree> tree(4, Tree(0,0));
	rep(i, 4)
	{
		tree[i] = Tree(s.size(), bases[i]);
		rep(j, sz(s)) tree[i].update(1, 0, n - 1, j, s[j]);
	}
	...
	while (q--)
	{
		...
		else
		{
			dread4(int, l, r, v, h);
			bool yes = 1;
			//cout << tree[0].query(1, 0, n - 1, l, r) << " " << tree[0].query(1, 0, n - 1, v, h) << "\n";
			rep(i, 4)
			{
				yes &= tree[i].query(1, 0, n - 1, l, r) == tree[i].query(1, 0, n - 1, v, h);
			}
			cout << (yes?"Ja":"Nej") << "\n";
		}
	}
