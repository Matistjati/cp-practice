...
int32_t main()
{
    ...
    repe(e, events)
    {
        if (get<2>(e)==0)
        {
            int t, k, _;
            tie(t, k, _) = e;
            t = lower_bound(all(vals), t) - begin(vals);
            quantity[t] += k;
        }
        else
        {
            int t1, t2, _;
            tie(t1, t2, _) = e;
            t1 = lower_bound(all(vals), t1) - begin(vals);
            t2 = lower_bound(all(vals), t2) - begin(vals);
            quantity[t2] += quantity[t1];
            quantity[t1] = 0;
        }
    }
