...
struct Tree
{
	...
	Tree(int n) : changes(n * 4) {}

	void update(int x, int l, int r, int ql, int qr, p2 q)
	{
		if (l > qr || r < ql) return;
		if (l >= ql && r <= qr) changes[x].push_back(q);
		else
		{
			int mid = (l + r) / 2;
			update(x * 2, l, mid, ql, qr, q);
			update(x * 2 + 1, mid + 1, r, ql, qr, q);
		}
	}

	void dfs(int x, UF& uf, int l, int r)
	{
		repe(q, changes[x]) uf.merge(q.first, q.second);
		if (l == r)
		{
			p2& q = isquery[l];
			if (q.first != -1) q.first = uf.find(q.first) == uf.find(q.second);
		}
		else
		{
			int mid = (l + r) / 2;
			dfs(x * 2, uf, l, mid);
			dfs(x * 2 + 1, uf, mid + 1, r);
		}
		repe(q, changes[x]) uf.rollback();
	}
};
...
int32_t main()
{
	...
	rep(i, q)
	{
		...
		if (t == 1)
		{
			assert(setcontains(waiting, p2(a, b)));
			intervals.emplace_back(a, b, waiting[p2(a, b)], i - 1);
			waiting.erase(p2(a, b));
		}
		if (t == 2)
		{
			isquery[i] = p2(a, b);
	...
	repe(w, waiting)
	{
		intervals.emplace_back(w.first.first, w.first.second, w.second, q - 1);
	}
	...
	repe(inter, intervals)
	{
		int a, b, l, r;
		tie(a, b, l, r) = inter;
		tree.update(1, 0, q - 1, l, r, p2(a, b));
	}
	...
	rep(i, q)
	{
		if (isquery[i].first != -1)
		{
			cout << (isquery[i].first ? "Ja" : "Nej") << "\n";
		}
	}
