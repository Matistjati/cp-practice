...
int knapsack(vi w, int t) {
    ...
    repp(i, b, sz(w)) {
        u = v;
        repp(x, 0, m) v[x + w[i]] = max(v[x + w[i]], u[x]);
        for (x = 2 * m; --x > m;) repp(j, max(0LL, u[x]), v[x])
...
int32_t main()
{
    ...
    vi nums2;
    vi items(1e6);
    repe(num, nums) items[num]++;
    rep(i, items.size())
    {
        while (items[i] >= 3)
        {
            items[i] -= 2;
            while (2 * i >= items.size()) items.resize(items.size()*2);
            items[2 * i]++;
        }
    }

    rep(item, items.size()) rep(i, items[item])
    {
        nums2.push_back(item);
    }
    cerr << nums2.size() << endl;
    cout << knapsack(nums2, t);
